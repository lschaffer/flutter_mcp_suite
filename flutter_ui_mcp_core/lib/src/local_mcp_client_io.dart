...
class LocalMCPClient extends MCPClient {
  ...
  LocalMCPClient(this.serverConfig, {super.logCallback}) : super(serverConfig.url);

  void _log(String message, {bool isError = false}) {
    if (logCallback != null) {
      logCallback!(message, isError: isError);
    } else {
      debugPrint('[LocalMCPClient] $message');
    }
  }
  ...
  Future<void> connect() async {
    try {
      ...
      final env = LocalMcpRuntime.augmentedEnv(baseEnv: serverConfig.localEnvVars);
      ...
      if (serverConfig.customLaunchCommand != null &&
        ...
        if (node == null) {
          throw const LocalMcpException('Node.js (18+) not found. Please install Node.js and restart the app.');
        }
        ...
        if (entry.toLowerCase().endsWith('.py') || entry.contains('.py')) {
          ...
          cmdArgs = ['-m', entry.isNotEmpty ? entry : serverConfig.name, ...args];
        }
      }
      ...
      final initResp = await _sendRpcRequest('initialize', {
        ...
        'capabilities': {},
        'clientInfo': {'name': 'Flutter Local MCP Client', 'version': '1.0.0'},
      });
      ...
      // 3. fetch tools list to populate _availableTools
      final toolsResp = await _sendRpcRequest('tools/list', null);
      final result = toolsResp['result'];
      if (result != null && result['tools'] is List) {
        _availableToolsLocal = (result['tools'] as List).map((tool) => MCPTool.fromJson(tool)).toList();
      }

      _isConnectedLocal = true;
      _log('Connected successfully via Stdio. Tools registered: ${_availableToolsLocal.length}');
      notifyListeners();
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _sendRpcRequest(
    ...
    return completer.future.timeout(timeout, onTimeout: () {
      sub.cancel();
      throw TimeoutException('Local MCP request "$method" timed out after ${timeout.inSeconds}s');
    });
  }
    ...
    try {
      ...
      if (resp.containsKey('error')) {
        ...
        return MCPToolResult(
          content: [MCPContent(type: 'text', text: 'Error (${err['code']}): ${err['message']}')],
          isError: true,
        );
      }
...
class LocalMcpRuntime {
  static Future<String> get _rootDir async {
    ...
    final dir = Directory(p.join(support.path, 'mcp-local-servers'));
  ...
  static Future<String?> _resolveViaShell(String toolName) async {
    ...
    try {
      final result = await Process.run('/bin/bash', ['-lc', 'command -v $toolName']);
      final resolved = (result.stdout as String?)?.trim() ?? '';
      if (result.exitCode == 0 && resolved.isNotEmpty) {
        return resolved;
      }
  ...
  static Map<String, String> augmentedEnv({Map<String, String>? baseEnv, String? extraPath}) {
  ...
  static List<String> buildLaunchArgs(McpServerConfig server) {
    ...
    final rawArgs = server.url.trim(); // We store launch args space-separated in URL field for local servers
  ...
  /// Installs the given local server and yields installation updates
  static Future<String?> install(
    ...
    if (method == 'uvx') {
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.detecting, 'Checking for uvx...'));
      final uvx = await detectUv();
      if (uvx == null) {
        return 'uvx executable not found. Please install uv and try again.';
      }
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.installing, 'Downloading package via uvx...'));
      // uvx installs on first launch or run
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.done, 'uvx server marked ready.'));
      return null;
    } else if (method == 'npx') {
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.detecting, 'Checking for Node.js...'));
      final node = await detectNode();
      if (node == null) {
        return 'Node.js (version 18+) not found. Please install Node.js and try again.';
      }
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.done, 'npx server marked ready.'));
      return null;
    } else if (method == 'npm') {
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.detecting, 'Checking for Node.js...'));
      final node = await detectNode();
      ...
      onProgress?.call(LocalInstallProgress(LocalInstallStep.installing, 'npm install -g $pkgName...'));
      try {
      ...
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.done, 'Package installed successfully.'));
      return null;
    } else {
      // pip virtualenv setup
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.detecting, 'Checking for Python 3...'));
      ...
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.installing, 'Creating virtual environment...'));
      try {
      ...
      onProgress?.call(LocalInstallProgress(LocalInstallStep.installing, 'pip install $pkgSpec...'));
      try {
        final isRequirements = pkgSpec.trim().toLowerCase().endsWith('.txt') || pkgSpec.contains('/requirements');
        final pipArgs = isRequirements ? ['install', '-r', pkgSpec] : ['install', pkgSpec];
      ...
      onProgress?.call(const LocalInstallProgress(LocalInstallStep.done, 'Virtual environment ready.'));
      return null;
    }
  }
