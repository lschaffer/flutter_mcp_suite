...
- Rebranded and modernized from `mcp_playground_flutter`.
- Drop-in conversational AI and Model Context Protocol (MCP) widget for Flutter.
- Added `FlutterUiMcp` as preferred class name, with backward-compatible `typedef FlutterUiMcp = McpPlayground;`.
...
- **Architecture modularization**: Extracted reusable Flutter UI components, dialogs, drawers, and state controllers into the [`mcp_playground_ui`](../flutter_ui_mcp_core) shared package.
- **Standalone GenUI separation**: Moved GenUI generative UI surfaces, catalog builders, and controllers into the dedicated [`genui_mcp_playground`](../flutter_genui_mcp) package.
